...
'''%(str(messages['address']), str(messages['metadata']), str(messages['name']), str(messages['symbol']), str(messages['total_supply']), str(Web3.toWei(messages['mint_price'], 'ether')))
...
def enterBid(_tokenId: uint256):
    ...
    assert self.sale_state == True
    assert _tokenId < self.totalSupply
    assert msg.sender != self.id_to_owner[_tokenId]
    assert msg.value != 0
...
@nonpayable
@external
def withdraw_owner(_amount: uint256):
    """
    @dev Withdraw from contract.
         Throws if "contract_state" is set to False.
         Throws if pending amount is 0.
...
'''%(str(messages['address']), str(messages['metadata']), str(messages['name']), str(messages['symbol']), str(messages['total_supply']), str(Web3.toWei(messages['mint_price'], 'ether')))
# contract = ""
# with open('./NFT.vy', 'r') as f:
#     contract = f.read()

print(type(contract2))

# contract.replace("<address>", str(messages['address']))
# contract.replace("<metadata>", str(messages['metadata']))
# contract.replace("name", str(messages['name']))
# contract.replace("symbol", str(messages['symbol']))
# contract.replace("<totalSupply>", str(messages['total_supply']))
# contract.replace("<mintPrice>", str(Web3.toWei(messages['mint_price'], 'ether')))

# contract%(str(messages['address']), str(messages['metadata']), str(messages['name']), str(messages['symbol']), str(messages['total_supply']), str(Web3.toWei(messages['mint_price'], 'ether')))

if messages['mark_feat'] == 1:
    with open('./final_contract.vy', 'w') as f:
        f.write(contract2)
else:
    with open('./final_contract.vy', 'w') as f:
        f.write(contract1)
